...
order_add_basket.addEventListener('click', () => {
    basket_element++;
    if (addCart.length > 0) {
        addCart.push({ image: '../images/image-product-1-thumbnail.jpg', basketLength: basket_element, heading: big_Text.textContent, productPrice: 125.00, orderCount: order_count + addCart[addCart.length-1].orderCount, multiply: 125.00 * (order_count + addCart[addCart.length-1].orderCount) });
    }else {
    ...
    if (addCart.length > 0) {
        ...
        basket_item_num.innerHTML = 1;

    }
    order_count_value.textContent = 1;
});
